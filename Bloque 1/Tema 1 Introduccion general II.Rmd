---
title: "Tema 1 Introduccion general II. Tipos de objetos."
author: "Julia Chacón"
date: "15/4/2020"
output:
  html_document:
...
Ya sabemos que `R` es un lenguaje orientado a objetos. Lo primero que os preguntaréis es qué es un objeto en `R`, y qué tipos de objetos existen. Los objetos son estructuras que combinan datos y/o funciones que operan sobre ellos. Por ejemplo el resultado de una función o de una suma un objeto. 
...
```{r Cómo crear vectores, echo = TRUE}
mivector <- c("julia","nagore","alejandro")
mivector
```

```{r Cómo crear vectores2, echo = TRUE}
...
```{r Cómo crear vectores3, echo = TRUE}
...
```{r Cómo crear vectores4, echo = TRUE}
...
```{r Cómo crear vectores5, echo = TRUE}
...
```{r Cómo crear vectores6, echo = TRUE}
v <- 1:10
v
length(v)
...
```{r Cómo acceder a los elementos de un vector, echo = TRUE}
...
```{r Cómo crear listas, echo = TRUE}
milista <- list("Quercus","Betula","Abies","Fagus","Taxus")  # llista de 5 elementos, cada uno de ellos es una especie
milista
```
...
```{r Como acceder a los elementos de una lista, echo = TRUE}
milista[1]
...
```{r Como acceder a los elementos de una lista2, echo = TRUE}
...
```{r Como acceder a los elementos de una lista3, echo = TRUE}
milista$sp1
milista[[1]]
```
...
```{r Como acceder a los elementos de una lista4, echo = TRUE}
milista[1]
```
...
```{r Como crear una matriz, echo=TRUE}
...
```{r Acceso a los elementos de una matriz, echo=TRUE}
...
```{r Cambiar valores dentro de una matriz, echo=TRUE}
...
```{r Cambiar valores dentro de una matriz2, echo=TRUE}
...
```{r Cambiar valores dentro de una matriz3, echo=TRUE}
...
```{r Como crear un data frame3, echo = TRUE, eval = FALSE}
...
```{r Como crear un data frame4, echo = TRUE, eval = FALSE}
...
```{r Como crear un data frame5, echo = TRUE, eval = FALSE}
...
```{r Acceso a los elementos de un data frame, echo = TRUE, eval = FALSE}
...
```{r Acceso a los elementos de un data frame2, echo = TRUE, eval = FALSE}
...
```{r Acceso a los elementos de un data frame3, echo = TRUE, eval = FALSE}
midataframe[,1] # le estaría diciendo: muestrame todas las filas, y la primera columna
...
```{rAcceso a los elementos de un data frame4, echo = TRUE, eval = FALSE}
